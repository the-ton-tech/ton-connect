...
- To hand off the request it generates a TonConnect deep link of the form  
  `tc://intent_inline?id=<client_pub_key>&r=<urlsafe(json.stringify(payload))>`, where:
    - `id` is the app client ID (public key) encoded as hex.
    - `r` is the payload encoded as URL-safe JSON string (same as `r` parameter in connect requests).
